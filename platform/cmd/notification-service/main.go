...
func main() {
	...
	routes := []server.Route{
		{Method: "POST", Path: "/v1/notifications", Handler: endpoint("notification:send", "notification.send")},
		{Method: "GET", Path: "/v1/notifications/stream", Handler: endpoint("notification:read", "notification.stream")},
	}
...
func respond(w http.ResponseWriter, r *http.Request, action string) {
	...
	_ = json.NewEncoder(w).Encode(map[string]any{"ok": true, "service": "notification-service", "action": action, "audit_event_id": evt.ID, "time": time.Now().UTC()})
}
