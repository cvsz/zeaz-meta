...
func main() {
	...
	routes := []server.Route{
		{Method: "POST", Path: "/v1/auth/register", Handler: endpoint("auth:public", "auth.register")},
		{Method: "POST", Path: "/v1/auth/login", Handler: endpoint("auth:public", "auth.login")},
		{Method: "GET", Path: "/v1/auth/me", Handler: endpoint("user:read:self", "auth.me")},
	}
...
func respond(w http.ResponseWriter, r *http.Request, action string) {
	...
	_ = json.NewEncoder(w).Encode(map[string]any{"ok": true, "service": "auth-service", "action": action, "audit_event_id": evt.ID, "time": time.Now().UTC()})
}
