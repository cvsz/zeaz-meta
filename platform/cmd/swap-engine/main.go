...
func main() {
	cfg := config.Load("swap-engine")
	routes := []server.Route{
		{Method: "POST", Path: "/v1/swaps/quotes", Handler: endpoint("swap:quote", "swap.quote")},
		{Method: "POST", Path: "/v1/swaps/orders", Handler: endpoint("swap:execute", "swap.order")},
	}
...
func respond(w http.ResponseWriter, r *http.Request, action string) {
	...
	_ = json.NewEncoder(w).Encode(map[string]any{"ok": true, "service": "swap-engine", "action": action, "audit_event_id": evt.ID, "time": time.Now().UTC()})
}
