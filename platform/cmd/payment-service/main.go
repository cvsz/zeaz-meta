...
func main() {
	...
	routes := []server.Route{
		{Method: "POST", Path: "/v1/payments/intents", Handler: endpoint("payment:create", "payment.intent.create")},
		{Method: "POST", Path: "/v1/webhooks/stripe", Handler: endpoint("payment:webhook", "payment.webhook.stripe")},
	}
...
func respond(w http.ResponseWriter, r *http.Request, action string) {
	...
	_ = json.NewEncoder(w).Encode(map[string]any{"ok": true, "service": "payment-service", "action": action, "audit_event_id": evt.ID, "time": time.Now().UTC()})
}
