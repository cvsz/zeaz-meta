...
func main() {
	...
	routes := []server.Route{
		{Method: "GET", Path: "/v1/audit/events", Handler: endpoint("audit:read", "audit.events.list")},
		{Method: "POST", Path: "/v1/audit/events", Handler: endpoint("audit:append", "audit.events.append")},
	}
...
func respond(w http.ResponseWriter, r *http.Request, action string) {
	...
	_ = json.NewEncoder(w).Encode(map[string]any{"ok": true, "service": "audit-service", "action": action, "audit_event_id": evt.ID, "time": time.Now().UTC()})
}
